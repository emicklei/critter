...
public class ProtobufPathTest {
    ...
    public void readProto() throws Exception {
        ProtoFile pf = ProtoParser.parseUtf8("test", getClass().getResourceAsStream("/sample.proto"));
 ...
 **/
        ...
        d.read(getClass().getResourceAsStream("/sample.proto"));
    ...
    public String value(UnknownFieldSet.Field f) {
        ...
        if (!f.getLengthDelimitedList().isEmpty()) {
            if (f.getLengthDelimitedList().size() == 1) {
                return dumpBytes(f.getLengthDelimitedList().get(0).toByteArray());
            }
